...
import { ChatFab, useChatInline, ChatInlinePanel } from '@/features/chat'
  ...
  useEffect(() => {
    if (article && viewedRef.current !== article.id) {
      ...
      apiPost(`/api/articles/${article.id}/read`)
        .then(() => globalMutate((key: string) => typeof key === 'string' && (key.startsWith('/api/feeds') || key === '/api/classification')))
        .catch(async () => {
          if (isFirstSeen) {
