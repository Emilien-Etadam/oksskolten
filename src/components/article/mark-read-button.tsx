...
export function MarkReadButton({ onClick, className = '', variant = 'round' }: MarkReadButtonProps) {
  ...
  return (
    <button
      ...
      className={
        isEdge
          ? `absolute inset-y-0 right-0 z-10 flex items-center justify-center overflow-hidden w-4 rounded bg-error text-error opacity-40 transition-[width,opacity,color] duration-200 ease-out motion-reduce:transition-none group-hover:w-[var(--mark-read-square,2.5rem)] group-hover:opacity-90 group-hover:text-white focus-visible:w-[var(--mark-read-square,2.5rem)] focus-visible:opacity-90 focus-visible:text-white focus-visible:outline-none ${className}`
