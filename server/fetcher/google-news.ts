 ...
 *
 * Three strategies, cheapest first: decode the token, follow the redirect, then
 * let FlareSolverr run the page's JavaScript (Google serves a script-driven
...
export async function resolveGoogleNewsUrl(url: string): Promise<string | null> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const fromHtml = extractPublisherUrl(await decodeResponse(res))
